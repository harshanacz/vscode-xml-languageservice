...
import { TokenType, ScannerState, Scanner } from '../xmlLanguageTypes.js';
	...
	function internalScan(): TokenType {
		...
		switch (state) {
			...
			case ScannerState.WithinContent:
				if (stream.advanceIfChar(_LAN)) { // <
					...
					if (stream.advanceIfChar(_QUE)) { // <?
						// Treat xml processing instructions like doctype/tags roughly
						state = ScannerState.WithinDoctype;
						return finishToken(offset, TokenType.StartDoctypeTag);
					}
			...
			case ScannerState.WithinTag:
				...
				if (stream.advanceIfChars([_QUE, _RAN])) { // ?>
					state = ScannerState.WithinContent;
					return finishToken(offset, TokenType.StartTagSelfClose);
				}
				if (stream.advanceIfChar(_RAN)) { // >
